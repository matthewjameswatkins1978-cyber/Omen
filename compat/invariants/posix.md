...
| `TERMINAL_FOREGROUND_PGRP_IS_JOB` | While the foreground job owns the terminal, terminal fg pgrp == job pgrp. | Strong: `tcgetpgrp` observation. |
| `SHELL_REGAINS_TTY_AFTER_JOB_STOP` | When the foreground job stops, the shell regains terminal foreground before presenting a prompt. | Strong: `tcgetpgrp` after stop == shell pgrp; prompt readiness only as secondary text signal. |
| `SHELL_REGAINS_TTY_AFTER_JOB_EXIT` | When the foreground job exits, the shell regains terminal foreground before the next prompt. | Strong: `tcgetpgrp` after exit == shell pgrp. |
| `WAIT_OBSERVES_STOPPED_STATE` | A stopped foreground child is observed as STOPPED (not only as a terminal exit). | Strong: wait `WUNTRACED` or Linux `/proc` state `T` / fixture STOPPING barrier corroborated. |
| `TERMINAL_SIGINT_TARGETS_FOREGROUND_JOB` | Ctrl-C / terminal-generated SIGINT reaches the foreground job rather than being mistaken for shell termination. | Strong: VINTR via PTY master; job signaled; shell alive; fg was job pgrp. Distinguish from programmatic `kill(pid)`. |
...
- wait status showing stopped/signaled: STRONG
- `tcgetpgrp`-style terminal observation: STRONG
